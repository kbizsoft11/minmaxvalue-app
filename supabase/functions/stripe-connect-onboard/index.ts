...
serve(async (req) => {
  ...
  try {
    ...
    const { account_type = "seller", organization_id } = await req.json();
    ...
    // For organization accounts, verify organization_id is provided
    ...
    if (!stripeAccountId) {
      ...
      const account = await stripe.accounts.create({
        ...
        settings: {
          payouts: {
            schedule: {
              interval: "manual", // Admin-controlled payouts
            },
          },
        },
      });
      ...
      if (insertError) {
        ...
        // Try to delete the Stripe account we just created
        try {
          await stripe.accounts.del(stripeAccountId);
        } catch (e) {
          console.error("Failed to cleanup Stripe account:", e);
        }
        throw new Error("Failed to save Stripe account");
      }
    }

    // Determine return URL based on account type
    const returnPath = account_type === "seller" 
      ? "/seller-dashboard" 
    ...
    const accountLink = await stripe.accountLinks.create({
      ...
      refresh_url: `${siteUrl}${returnPath}?stripe_refresh=true`,
      return_url: `${siteUrl}${returnPath}?stripe_onboarded=true`,
